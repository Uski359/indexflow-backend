...
const addressWithDefault = (defaultValue) => z
    .string()
    .optional()
    .transform((val) => {
    if (!val) {
...
const indexNodeEnvSchema = z
    ...
    INDEX_POLL_INTERVAL_MS: z.coerce.number().int().min(1000).default(4000),
    HEALTH_LOG_INTERVAL_MS: z.coerce.number().int().min(1000).default(15000),
    ...
    COORDINATOR_PRIVATE_KEY: z
        ...
        .optional(),
    COORDINATOR_BRIDGE_INTERVAL_MS: z.coerce.number().int().min(1000).default(30000),
    ...
    RPC_COOLDOWN_MS: z.coerce.number().int().min(1000).default(60000)
})
    .transform((raw) => {
    const rpcUrls = raw.RPC_URLS && raw.RPC_URLS.trim().length > 0
        ? raw.RPC_URLS.split(',')
            .map((url) => url.trim())
...
export const loadIndexNodeEnv = (source = process.env) => indexNodeEnvSchema.parse(source);
