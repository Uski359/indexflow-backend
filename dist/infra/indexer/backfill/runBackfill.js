...
const fetchLogs = async (chainId, provider, address, topics, fromBlock, toBlock) => withRetry(() => provider.getLogs({
    address,
    topics,
    fromBlock,
    toBlock
}), { taskName: `${chainId}:getLogs:${fromBlock}-${toBlock}`, logger }).catch((error) => {
    logger.error('Failed to fetch logs; skipping range', { chainId, err: error, fromBlock, toBlock });
    return [];
});
const createTimestampResolver = (provider) => {
    const cache = new Map();
    return async (blockNumber) => {
        if (cache.has(blockNumber)) {
            return cache.get(blockNumber);
        }
        const block = await withRetry(() => provider.provider.getBlock(blockNumber), { taskName: `${provider.chain.id}:getBlock:${blockNumber}`, logger }).catch((error) => {
...
const backfillContract = async ({ chainId, provider, target, topics, latest, label, onLog }) => {
    let total = 0;
...
export const runBackfill = async (chainId) => {
