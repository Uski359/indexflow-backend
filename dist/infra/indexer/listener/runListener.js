import '../env.js';
import { getContributionsCollection, getPoiEventsCollection, getStakingEventsCollection } from '../db/mongo.js';
...
import { parseStakingEvent, REWARD_TOPIC, STAKED_TOPIC, UNSTAKED_TOPIC } from '../parsers/staking.js';
import { parseProofSubmitted, PROOF_SUBMITTED_TOPIC } from '../parsers/poi.js';
import { CONTRIBUTION_RECORDED_TOPIC, parseContributionRecorded } from '../parsers/contributions.js';
import { getProvider, obfuscateRpcUrl } from '../services/provider.js';
import { withRetry } from '../utils/retry.js';
import { loadLastProcessedBlock, persistLastProcessedBlock } from '../db/state.js';
const requireChainReady = (chainId) => {
...
const resolveTarget = (label, target, fallbackDeployBlock) => {
    if (!target?.address) {
        return null;
    }
...
const getLogIndex = (log) => log.logIndex ?? log.index ?? 0;
const fetchLogs = async (chainId, provider, address, topics, blockNumber) => withRetry(() => provider.getLogs({
    fromBlock: blockNumber,
    toBlock: blockNumber,
    address,
    topics
}), { taskName: `${chainId}:getLogs:${blockNumber}`, logger }).catch((error) => {
    logger.error('Failed to fetch block logs; skipping block', { chainId, err: error, blockNumber });
    return [];
});
const POLL_INTERVAL_MS = Math.max(1000, Number(process.env.INDEXER_POLL_INTERVAL_MS ?? '5000'));
const LISTENER_BATCH_SIZE = Math.max(1, Number(process.env.INDEXER_BATCH_SIZE ?? '25'));
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
export const runListener = async (chainId) => {
    ...
    const contributionTarget = resolveTarget('contributionRegistry', chain.utility?.contributionRegistry, chain.deployBlock);
    ...
    const resolveStartingBlock = async () => {
    ...
    const processBlock = async (blockNumber) => {
        const block = await withRetry(() => provider.provider.getBlock(blockNumber), { taskName: `${chain.id}:getBlock:${blockNumber}`, logger }).catch((error) => {
        ...
        const transferLogs = await fetchLogs(chain.id, provider, chain.tokenAddress, [TRANSFER_TOPIC], blockNumber);
        let stakingCount = 0;
        let poiCount = 0;
        ...
        if (stakingReady) {
            const stakingLogs = await fetchLogs(chain.id, provider, stakingTarget.address, [[STAKED_TOPIC, UNSTAKED_TOPIC, REWARD_TOPIC]], blockNumber);
            for (const log of stakingLogs) {
                const parsed = parseStakingEvent(log);
                const logIndex = getLogIndex(log);
                await withRetry(() => stakingCollection.updateOne({ chain: chain.id, txHash: log.transactionHash, logIndex }, {
                    $set: {
                        chain: chain.id,
                ...
                }, { upsert: true }), { taskName: `mongo:staking:${chain.id}:${log.transactionHash}:${logIndex}`, logger, baseDelayMs: 300 }).catch((error) => {
        ...
        if (poiReady) {
            const poiLogs = await fetchLogs(chain.id, provider, poiTarget.address, [PROOF_SUBMITTED_TOPIC], blockNumber);
            for (const log of poiLogs) {
                const parsed = parseProofSubmitted(log);
                const logIndex = getLogIndex(log);
                await withRetry(() => poiCollection.updateOne({ chain: chain.id, txHash: log.transactionHash, logIndex }, {
                    $set: {
                        chain: chain.id,
                        block: Number(log.blockNumber),
                        txHash: log.transactionHash,
                ...
                }, { upsert: true }), { taskName: `mongo:poi:${chain.id}:${log.transactionHash}:${logIndex}`, logger, baseDelayMs: 300 }).catch((error) => {
        ...
        const contributionsReady = contributionTarget && blockNumber >= contributionTarget.deployBlock;
        if (contributionsReady) {
            const contributionLogs = await fetchLogs(chain.id, provider, contributionTarget.address, [CONTRIBUTION_RECORDED_TOPIC], blockNumber);
            for (const log of contributionLogs) {
                const parsed = parseContributionRecorded(log);
                const logIndex = getLogIndex(log);
                await withRetry(() => contributionsCollection.updateOne({ chain: chain.id, txHash: log.transactionHash, logIndex }, {
                    $set: {
                        chain: chain.id,
                        block: Number(log.blockNumber),
                        txHash: log.transactionHash,
