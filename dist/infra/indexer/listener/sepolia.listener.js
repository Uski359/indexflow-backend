import '../env.js';
import { Interface, JsonRpcProvider, getAddress, id, isError } from 'ethers';
import { MongoClient, MongoServerError } from 'mongodb';
import pino from 'pino';
import { TransferSchema } from '../../../api/schema/transferSchema.js';
...
const obfuscateRpcUrl = (url) => url.replace(/(https?:\/\/)([^@]+)@/u, '$1***@');
...
class MultiRpcProvider {
    constructor(rpcUrls, log = logger, { baseDelayMs = 500, maxDelayMs = 10000, backoffFactor = 2, maxAttempts = 5 } = {}) {
        this.log = log;
        this.currentIndex = 0;
    ...
    async callWithRetry(taskName, fn) {
        let attempt = 0;
        ...
        while (attempt < this.maxAttempts) {
            ...
            try {
                ...
                if (attempt > 0) {
                    this.log.info({ taskName, attempt: attempt + 1, rpc: obfuscateRpcUrl(this.getCurrentRpc()) }, 'Recovered after RPC retry');
                }
                return result;
            }
            catch (error) {
                ...
                const shouldRotate = rateLimited ||
        ...
        this.log.error({ taskName, attempts: this.maxAttempts, err: lastError }, 'RPC call exhausted retries');
        throw lastError instanceof Error ? lastError : new Error('RPC call failed');
    }
    createProvider(url) {
        const provider = new JsonRpcProvider(url);
        provider.on('error', (error) => {
            this.log.error({ err: error, rpc: obfuscateRpcUrl(url) }, 'Provider emitted an error; rotating');
            this.rotateProvider(error);
        });
        return provider;
    }
...
class TransferRepository {
    constructor(uri, dbName, log = logger) {
        this.uri = uri;
        this.dbName = dbName;
        this.log = log;
    ...
    async getCollection() {
        if (!this.connected) {
            ...
            this.collection = this.client.db(this.dbName).collection('transfers');
            await this.collection.createIndex({ chainId: 1, txHash: 1, logIndex: 1 }, {
    ...
    async insert(transfer) {
        ...
        const doc = { ...transfer, chain: transfer.chainId };
        try {
            ...
            if (error instanceof MongoServerError && error.code === 11000) {
                this.log.debug({ chainId: doc.chainId, txHash: doc.txHash, logIndex: doc.logIndex }, 'Duplicate transfer ignored');
                return;
            }
            throw error;
        }
...
class IndexStateRepository {
    ...
    async getCollection() {
        if (!this.connected) {
            await this.client.connect();
            this.collection = this.client.db(this.dbName).collection('indexer_state');
    ...
    async setLastProcessedBlock(chainId, block, currentChainBlock) {
        const collection = await this.getCollection();
        const stateUpdate = {
            chainId,
        ...
        await collection.updateOne({ chainId }, { $set: stateUpdate }, { upsert: true });
    }
}
const loadRpcUrls = () => [process.env.SEPOLIA_RPC_1, process.env.SEPOLIA_RPC_2, process.env.SEPOLIA_RPC_3]
...
export const startListener = async () => {
    ...
    let lastProcessedBlock = (await stateRepo.getLastProcessedBlock(chainId)) ??
        Number(process.env.SEPOLIA_START_BLOCK ?? 0);
    if (lastProcessedBlock === 0) {
        const current = await multiProvider.callWithRetry('getBlockNumber:init', (p) => p.getBlockNumber());
    ...
    while (true) {
        try {
            const currentBlock = Number(await multiProvider.callWithRetry('getBlockNumber', (provider) => provider.getBlockNumber()));
            ...
            const logs = await multiProvider.callWithRetry('getLogs', (provider) => provider.getLogs({
                address: tokenAddress,
                fromBlock,
                toBlock,
                topics: [TRANSFER_TOPIC]
            }));
            logger.info({ fromBlock, toBlock, logs: logs.length }, 'Fetched logs for range');
            for (const log of logs) {
                try {
                    ...
                    const block = await multiProvider.callWithRetry('getBlock', (provider) => provider.getBlock(blockNumber));
                    const transfer = {
                        chainId: chainId,
                        blockNumber,
                        txHash,
                    ...
                    if (!validation.success) {
                        logger.error({ txHash, blockNumber, issues: validation.error.issues }, 'Transfer validation failed');
                        continue;
                    }
...
void startListener().catch((error) => {
