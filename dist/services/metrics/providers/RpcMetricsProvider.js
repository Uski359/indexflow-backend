...
import { resolveRpcEnvKey, resolveRpcUrl } from '../../../infra/rpc/getLatestBlock.js';
import { MetricsNotAvailableError } from '../errors.js';
import { areTargetsErc20Tagged, toUnixSeconds, resolveTargets } from '../providerInput.js';
const TRANSFER_TOPIC = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef';
const DAY_SECONDS = 24 * 60 * 60;
const MAX_TARGETS = 20;
const MAX_BLOCK_RANGE = 200000;
const LOG_BLOCK_CHUNK = 5000;
...
const getTimestampForBlock = async (provider, blockNumber, cache) => {
...
const findBlockAtOrBefore = async (provider, low, high, timestamp, cache) => {
    let left = low;
    let right = high;
    let resolved = low - 1;
    while (left <= right) {
...
const findBlockAtOrAfter = async (provider, low, high, timestamp, cache) => {
    let left = low;
    let right = high;
    let resolved = high + 1;
    while (left <= right) {
...
const collectTransferLogs = async (provider, target, walletTopic, fromBlock, toBlock) => {
...
export class RpcMetricsProvider {
    async getWalletMetrics(input) {
        ...
        if (!rpcUrl) {
            throw new MetricsNotAvailableError(`rpc_missing:${resolveRpcEnvKey(input.chain_id) ?? 'RPC_URL_<CHAIN>'}`);
        }
        ...
        try {
            ...
            const asOfBlock = Math.min(latestBlock, Math.max(0, Math.trunc(input.as_of_block ?? latestBlock)));
            const minBlock = Math.max(0, asOfBlock - MAX_BLOCK_RANGE);
            const minBlockTimestamp = await getTimestampForBlock(provider, minBlock, blockTimestampCache);
            if (startTimestamp < minBlockTimestamp) {
                throw new MetricsNotAvailableError('rpc_range_too_large');
            }
            const asOfTimestamp = await getTimestampForBlock(provider, asOfBlock, blockTimestampCache);
            const boundedEndTimestamp = Math.min(endTimestamp, asOfTimestamp);
            const fromBlock = await findBlockAtOrAfter(provider, minBlock, asOfBlock, startTimestamp, blockTimestampCache);
            const toBlock = await findBlockAtOrBefore(provider, minBlock, asOfBlock, boundedEndTimestamp, blockTimestampCache);
            ...
            const matchedLogs = new Map();
            for (const target of targets) {
                // eslint-disable-next-line no-await-in-loop
                const targetLogs = await collectTransferLogs(provider, target, walletTopic, fromBlock, toBlock);
            ...
            for (const blockNumber of matchedLogs.values()) {
                // eslint-disable-next-line no-await-in-loop
                const timestamp = await getTimestampForBlock(provider, blockNumber, blockTimestampCache);
