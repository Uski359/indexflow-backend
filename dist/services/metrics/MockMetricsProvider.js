...
const resolveWindowType = (start, end) => {
    const delta = end - start;
    const entries = Object.entries(WINDOW_SECONDS);
...
const generateDeterministicSummary = (campaign_id, start, end, wallet) => {
...
export class MockMetricsProvider {
    async getWalletMetrics(input) {
        ...
        return generateDeterministicSummary(campaign_id, input.start, input.end, normalizedWallet);
    }
}
export const getMockMetricsDatasetPath = () => cachedDatasetPath ?? resolveDatasetPath();
