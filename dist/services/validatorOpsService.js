...
function toValidatorMetadata(metadata) {
    return {
        name: metadata.name,
...
function mapProofGenerationResult(payload, raw) {
    return {
...
export async function generateProof(payload) {
    ...
    const result = await postValidator('/proof/generate', body);
    return mapProofGenerationResult(payload, result);
}
export async function scheduleProofSubmission(payload) {
    const targetChainId = payload.chainId ?? config.chainId;
    const body = {
        dataset_id: payload.datasetId,
        validator: payload.validator,
    ...
    const job = await postValidator('/proof/schedule', body);
    return mapProofJob(job);
}
export async function listProofJobs() {
    const jobs = await callValidator('/proof/jobs');
    return jobs.map(mapProofJob);
}
export async function updateProofJob(jobId, payload) {
    const body = {
        status: payload.status,
        tx_hash: payload.txHash,
        error: payload.error
    };
    const job = await callValidator(`/proof/jobs/${jobId}`, {
        method: 'PATCH',
