...
import { getProvider } from '../../../infra/indexer/services/provider.js';
import { logger } from '../../../infra/config/logger.js';
const TRANSFERS_COLLECTION = 'transfers';
const DEFAULT_CHAIN = 'sepolia';
export class HealthService {
    static async getHealth(chain) {
        ...
        const [latest, state] = await Promise.all([
            transfers
                ...
                .toArray()
                .then((items) => items[0]),
            db.collection('indexer_state').findOne({ chainId }, { sort: { updatedAt: -1 } })
        ]);
        const latestIndexedBlock = state?.lastProcessedBlock ?? latest?.blockNumber ?? latest?.block ?? null;
        let providerBlock = state?.currentChainBlock ?? null;
        ...
        const lag = providerBlock !== null && latestIndexedBlock !== null
