...
import { connectDB, getTransfersCollection } from '../../../infra/indexer/db/mongo.js';
import { StatsService } from '../services/stats.service.js';
export const getIndexerStats = async (req, res, next) => {
    try {
        const [db, transfers] = await Promise.all([connectDB(), getTransfersCollection()]);
        const [states, transferGroups] = await Promise.all([
            db.collection('indexer_state').find({}).toArray(),
            transfers
                .aggregate([
                {
                    $group: {
        ...
        const transfersByChain = new Map();
        ...
        const chains = states.map((state) => {
            ...
            const lag = currentChainBlock !== null && lastIndexedBlock !== null
...
export const getThroughputStats = async (req, res, next) => {
    try {
        const chain = req.query.chain || undefined;
