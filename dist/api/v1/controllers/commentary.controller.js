...
export const postCampaignCommentary = async (req, res, next) => {
    try {
        const body = req.body;
        ...
        const commentaryResults = await Promise.all(result.results.map(async (entry) => {
            const commentaryResult = await commentaryService.getCommentaryForOutput(entry.output, entry.insights);
            return {
                wallet: entry.wallet,
                output: entry.output,
